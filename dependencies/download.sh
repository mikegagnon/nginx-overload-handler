...
wget --output-document=$AUTOCONF_DL_LOCAL_PATH $AUTOCONF_DL_REMOTE_PATH
tar -zxvf $AUTOCONF_DL_LOCAL_PATH

wget --output-document=$PHP_APC_DL_LOCAL_PATH $PHP_APC_DL_REMOTE_PATH
cp $PHP_APC_DL_LOCAL_PATH $PHP_VULN_DL_LOCAL_PATH/ext
cd $PHP_VULN_DL_LOCAL_PATH/ext
tar -zxvf APC-3.1.10.tgz
mv package.xml APC-3.1.10
mv APC-3.1.10 apc
rm APC-3.1.10.tgz
